...
class PFLDLoss(nn.Module):
    ...
    def forward(self, attribute_gt, landmark_gt, euler_angle_gt, euler_angle,
                landmarks, train_batchsize):
        weight_angle = torch.sum(1 - torch.cos(euler_angle - euler_angle_gt), axis=1)
...
class WingLoss(nn.Module):
    def __init__(self, w=10.0, epsilon=2.0, N_LANDMARK=98):
        super(WingLoss, self).__init__()
        self.w = w
        self.epsilon = epsilon
