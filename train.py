...
def train(train_loader, pfld_backbone, auxiliarynet, criterion, optimizer,
    ...
    for iteration, (img, landmark_gt, attribute_gt, euler_angle_gt) in enumerate(train_loader):
        ...
        euler_angle = auxiliarynet(features)
        weighted_loss, loss = criterion(attribute_gt, landmark_gt,
                                        euler_angle_gt, euler_angle, landmarks,
...
def main(args):
    ...
    if os.path.exists(args.resume) and args.resume.endswith('.pth'):
        ...
        pfld_backbone.load_state_dict(check["plfd_backbone"])
    ...
    for epoch in range(args.start_epoch, args.end_epoch + 1):
        ...
        val_loss, cur_val_nme = validate(wlfw_val_dataloader, pfld_backbone, auxiliarynet, criterion)
